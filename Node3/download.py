...
def download_pieces_concurrently(file_info, output_file):
    threads = []
    piece_status = file_info.get("piece_status", [])

    for piece_index, status in enumerate(piece_status):
        if status == 0:  # Only download pieces that are missing
            peers = file_info["pieces"].get(str(piece_index), [])
            if not peers:
                continue
            peer = peers[0]
            peer_ip, peer_port = peer.split(":")  

            thread = threading.Thread(target=download_piece_and_write_to_file, args=(peer_ip, int(peer_port), piece_index, output_file))
